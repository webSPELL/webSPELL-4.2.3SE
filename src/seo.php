...
switch ($GLOBALS['site']) {
	...
	case 'data':
		...
		if(isset($_GET['id'])){
		  $id=(int)$_GET['id'];
		  $db_name=safe_query("SELECT `name` FROM `".PREFIX."database` WHERE `databaseID`=".$id);
		  $any_data=mysql_num_rows($db_name);
		}
		if($any_data){
		  $dbname=mysql_fetch_array($db_name);
		  define('PAGETITLE', settitle($_language->module['database'].' - '.$dbname['name']));	
		}
		else{
		  define('PAGETITLE', settitle($_language->module['database']));
		}
		break;
