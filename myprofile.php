...
if(!$userID) echo $_language->module['not_logged_in'];
	...
	if(isset($_POST['submit'])) {
		...
		if(isset($_POST['private_data_hide'])) $private_data_hide = 0;
		else $private_data_hide = 1;
		...
		else
		{
			safe_query("UPDATE `".PREFIX."user`
						SET 
							...
							publishprivatedata='".$private_data_hide."',
							firstname='".$firstname."',
							lastname='".$lastname."',
							sex='".$sex."',
							country='".$flag."',
			...
			if($editname) safe_query("UPDATE ".PREFIX."user SET username='".$usernamenew."' WHERE userID='".$id."'");
	...
	elseif(isset($_POST['savemail'])){
		...
		if($anz) {
			...
			if($ds['publishprivatedata']) $private_data_hide = '';
			else $private_data_hide = ' checked="checked"';
