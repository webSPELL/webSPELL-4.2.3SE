...
if(isset($_POST['save'])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		safe_query("INSERT INTO ".PREFIX."servers ( name, ip, game, info ) values( '".$_POST['name']."', '".$_POST['serverip']."', '".$_POST['server_type']."', '".$_POST['message']."' ) ");
...
elseif(isset($_POST['saveedit'])) {
 	$CAPCLASS = new Captcha;
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		safe_query("UPDATE ".PREFIX."servers SET name='".$_POST['name']."', ip='".$_POST['serverip']."', game='".$_POST['server_type']."', info='".$_POST['message']."' WHERE serverID='".$_POST['serverID']."'");
	} else echo $_language->module['transaction_invalid'];
}
...
elseif($action=="edit") {
  ...
  $serverID = $_GET['serverID'];
	...
	$servertype=array('ts'=>'Teamspeak', 'vent'=>'Ventrilo', 'mumble'=>'Mumble');
  echo '<form method="post" id="post" name="post" action="admincenter.php?site=servers" onsubmit="return chkFormular();">
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr>
      <td width="15%"><b>'.$_language->module['server_name'].'</b></td>
      <td width="85%"><input type="text" name="name" size="60" value="'.htmlspecialchars($ds['name']).'" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['game'].'</b></td>
      <td>
  ...
  echo ' </select>
    ...
    <tr>
      <td><b>'.$_language->module['ip_port'].'</b></td>
      <td><input type="text" name="serverip" size="60" value="'.$ds['ip'].'" /></td>
    </tr>
    <tr>
      <td colspan="2" valign="top"><b>'.$_language->module['info'].'</b>
        <table width="99%" border="0" cellspacing="0" cellpadding="0">
        ...
        <br /><textarea id="message" name="message" rows="5" cols="" style="width: 100%;">'.htmlspecialchars($ds['info']).'</textarea>
      </td>
    </tr>
    <tr>
  ...
  echo'<form method="post" name="ws_servers" action="admincenter.php?site=servers">
    ...
    while($ds=mysql_fetch_array($ergebnis)) {
      ...
      if($i%2) { $td='td1'; }
			...
			echo'<tr>
        <td class="'.$td.'"><img src="../images/games/'.$ds['game'].'.gif" width="13" height="13" border="0" alt="" /> <a href="'.$protocol.$ds['ip'].'"><b>'.$ds['ip'].'</b></a><br /><b>'.htmlspecialchars($ds['name']).'</b><br />'.cleartext($ds['info'],1,'admin').'</td>
