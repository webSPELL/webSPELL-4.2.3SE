...
elseif(isset($_POST['saveedit'])) {
 	$CAPCLASS = new Captcha;
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		safe_query("UPDATE ".PREFIX."links_categorys SET name='".$_POST['name']."' WHERE linkcatID='".$_POST['linkcatID']."'");
	} else echo $_language->module['transaction_invalid'];
}
...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr>
      <td width="15%"><b>'.$_language->module['category_name'].'</b></td>
      <td width="85%"><input type="text" name="name" value="'.htmlspecialchars($ds['name']).'" size="60" /></td>
    </tr>
    <tr>
