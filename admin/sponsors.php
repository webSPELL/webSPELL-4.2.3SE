...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    ...
    <tr>
      <td><b>'.$_language->module['sponsor_name'].'</b></td>
      <td><input type="text" name="name" size="60" maxlength="255" value="'.htmlspecialchars($ds['name']).'" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['sponsor_url'].'</b></td>
      <td><input type="text" name="url" size="60" value="'.$ds['url'].'" /></td>
    </tr>
    <tr>
      <td colspan="2">
        <b>'.$_language->module['description'].'</b>
        ...
        <br /><textarea id="message" rows="5" cols="" name="message" style="width: 100%;">'.htmlspecialchars($ds['info']).'</textarea>
      </td>
    </tr>
    <tr>
      <td><b>'.$_language->module['is_displayed'].'</b></td>
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#DDDDDD">
    ...
    while($ds = mysql_fetch_array($qry)) {
      if($i%2) { $td='td1'; }
			...
			else $name='<a href="http://'.$ds['url'].'" target="_blank">'.$ds['name'].'</a>';
