...
elseif($action=="add") {
  ...
  $ergebnis=safe_query("SELECT * FROM ".PREFIX."forum_categories ORDER BY sort");
	...
	while($ds=mysql_fetch_array($ergebnis)) {
		$cats.='<option value="'.$ds['catID'].'">'.htmlspecialchars($ds['name']).'</option>';
	}
	$cats.='</select>';
	...
	while($db=mysql_fetch_array($sql)) {
		$groups.='<option value="'.$db['fgrID'].'">'.htmlspecialchars($db['name']).'</option>';
	}
	$CAPCLASS = new Captcha;
	$CAPCLASS->create_transaction();
	$hash = $CAPCLASS->get_hash();
...
elseif($action=="edit") {
  ...
  $boardID = $_GET['boardID'];
	...
	while($dc=mysql_fetch_array($category)) {
		...
		$cats.='<option value="'.$dc['catID'].'"'.$selected.'>'.htmlspecialchars($dc['name']).'</option>';
	}
	$cats.='</select>';
	...
	foreach($groups as $fgrID => $name) {
		...
		$readgrps .= '<option value="'.$fgrID.'"'.$selected.'>'.htmlspecialchars($name).'</option>';
	}
	...
	foreach($groups as $fgrID => $name) {
		...
		$writegrps .= '<option value="'.$fgrID.'"'.$selected.'>'.htmlspecialchars($name).'</option>';
	}
	$CAPCLASS = new Captcha;
	$CAPCLASS->create_transaction();
	$hash = $CAPCLASS->get_hash();
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    ...
    <tr>
      <td><b>'.$_language->module['boardinfo'].'</b></td>
      <td><input type="text" name="boardinfo" value="'.htmlspecialchars($ds['info']).'" size="60" /></td>
    </tr>
    <tr>
      <td valign="top"><b>'.$_language->module['read_right'].'</b></td>
...
elseif($action=="editcat") {
  ...
  echo'<h1>&curren; <a href="admincenter.php?site=boards" class="white">'.$_language->module['boards'].'</a> &raquo; '.$_language->module['edit_category'].'</h1>';
	...
	while($db=mysql_fetch_array($sql)) {
		...
		$groups.='<option value="'.$db['fgrID'].'" '.$selected.'>'.htmlspecialchars($db['name']).'</option>';
	}
	$groups.='</select>';
  $CAPCLASS = new Captcha;
	$CAPCLASS->create_transaction();
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr>
      <td width="15%"><b>'.$_language->module['category_name'].'</b></td>
      <td width="85%"><input type="text" name="catname" value="'.htmlspecialchars($ds['name']).'" size="60" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['category_info'].'</b></td>
      <td><input type="text" name="catinfo" value="'.htmlspecialchars($ds['info']).'" size="60" /></td>
    </tr>
    <tr>
      <td valign="top"><b>'.$_language->module['read_right'].'</b></td>
      <td>'.$groups.'<br />
...
echo'<h1>&curren; '.$_language->module['boards'].'</h1>';
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#DDDDDD">
    ...
    echo'<tr bgcolor="#CCCCCC">
      <td class="td_head"><b>'.$ds['name'].'</b><br /><small>'.$ds['info'].'</small></td>
      <td class="td_head"></td>
	...
	while($db=mysql_fetch_array($boards)) {
      ...
      <td bgcolor="#FFFFFF"><b>'.$db['name'].'</b></td>
