...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    ...
    <tr>
      <td><b>'.$_language->module['banner_name'].'</b></td>
      <td><input type="text" name="bannername" size="60" maxlength="255" value="'.htmlspecialchars($ds['bannername']).'" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['banner_url'].'</b></td>
      <td><input type="text" name="bannerurl" size="60" value="'.$ds['bannerurl'].'" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['is_displayed'].'</b></td>
      <td>'.$displayed.'</td>
  ...
  $CAPCLASS = new Captcha;
    ...
    while($ds = mysql_fetch_array($qry)) {
      if($i%2) { $td='td1'; }
			...
			else $bannerurl='<a href="http://'.$ds['bannerurl'].'" target="_blank">'.$ds['bannerurl'].'</a>';
