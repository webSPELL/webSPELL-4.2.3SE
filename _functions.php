...
function clearbbcodehtml($text){
  return strip_tags(htmloutput($text), '<br><br />');
}
...
if(date("dh",$lastBanCheck) != date("dh")){
	...
	if(!empty($removeBan)){
		...
		safe_query("UPDATE ".PREFIX."user SET banned='' WHERE ".$where);
	}
...
if($userID) safe_query("UPDATE ".PREFIX."user SET ip='".$GLOBALS['ip']."' WHERE userID='".$userID."'");
