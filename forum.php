...
function showboard($board) {
	...
	$actions='<input type="button" value="'.$_language->module['search'].'" onclick="window.location=\'index.php?site=search\'" />';
	if($loggedin) {
		$mark='&#8226; <a href="index.php?site=forum&amp;board='.$board.'&amp;action=markall">'.$_language->module['mark_topics_read'].'</a>';
		if($writer) $actions.=' <input type="button" value="'.$_language->module['new_topic'].'" onclick="window.location=\'index.php?site=forum&amp;addtopic=true&amp;board='.$board.'\'" />';
	} else $mark='';
...
if(isset($_POST['submit']) || isset($_POST['movetopic']) || isset($_GET['addtopic']) || isset($_POST['addtopic']) || (isset($_GET['action']) and $_GET['action'] == "admin-action") || isset($_POST['admaction'])) {
	...
	elseif(isset($_REQUEST['addtopic'])) {
		...
		if($loggedin) {
          ...
          <tr bgcolor="'.BGHEAD.'">
            <td colspan="2" class="title" align="left"><b>'.cleartext($topicname).'</b></td>
          </tr>';
