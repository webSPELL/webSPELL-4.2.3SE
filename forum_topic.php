...
function showtopic($topic, $edit, $addreply, $quoteID, $type) {
  ...
  $topicactions='';
	...
	if($loggedin and $writer) $topicactions.='<input type="button" value="'.$_language->module['new_topic'].'" onclick="window.location=\'index.php?site=forum&amp;addtopic=true&amp;action=newtopic&amp;board='.$dt['boardID'].'\'" /> <input type="button" value="'.$_language->module['new_reply'].'" onclick="window.location=\'index.php?site=forum_topic&amp;topic='.$topic.'&amp;addreply=true&amp;page='.$pages.'&amp;type='.$type.'\'" />';
  ...
  eval ("\$forum_topics_title = \"".gettemplate("forum_topics_title")."\";");
	...
	while($dr=mysql_fetch_array($replys)) {
		...
		if(!$dt['closed']) $quote='<input type="button" value="'.$_language->module['quote'].'" onclick="window.location=\'index.php?site=forum_topic&amp;addreply=true&amp;board='.$dt['boardID'].'&amp;topic='.$topic.'&amp;quoteID='.$dr['postID'].'&amp;page='.$page.'&amp;type='.$type.'\'" />';
		else $quote = "";
		...
		$actions='';
		if(($userID == $dr['poster'] OR isforumadmin($userID) OR ismoderator($userID,$dt['boardID']))&& !$dt['closed']) $actions=' <input type="button" value="'.$_language->module['edit'].'" onclick="window.location=\'index.php?site=forum_topic&amp;topic='.$topic.'&amp;edit=true&amp;id='.$dr['postID'].'&amp;page='.$page.'\'" /> ';
