...
if(isset($id) and getnickname($id) != '') {
	...
	elseif($action == "galleries") {
		...
		echo '<br /><table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#DDDDDD">
      <tr>
        <td class="title" colspan="4"><b>'.$_language->module['galleries'].' '.$_language->module['by'].' '.getnickname($id).'</b></td>
      </tr>
      <tr bgcolor="'.$bgcat.'">
        <td width="100">&nbsp;</td>
	...
	elseif($action == "lastposts") {
		...
		if(mysql_num_rows($topics)) {
            ...
            <table width="100%" cellpadding="2" cellspacing="0">
              <tr>
                <td colspan="3"><div style="width: 250px; overflow:hidden;"><a href="index.php?site=forum_topic&amp;topic='.$db['topicID'].'">'.$posttime.'<br /><b>'.clearfromtags($db['topic']).'</b></a><br /><i>'.$db['views'].' '.$_language->module['views'].' - '.$db['replys'].' '.$_language->module['replys'].'</i></div></td>
              </tr>
            </table>
            </td>
          </tr>';
		...
		if(mysql_num_rows($posts)) {
			...
			while($db = mysql_fetch_array($posts)) {
				...
				$n % 2 ? $bgcolor1 = BG_1 : $bgcolor1 = BG_1;
				$n % 2 ? $bgcolor2 = BG_1 : $bgcolor2 = BG_1;
				$posttime = date("d.m.y h:i", $db['date']);
	...
	elseif($action == "guestbook") {
		...
		elseif(isset($_GET['delete'])) {
			...
			while ($ds = mysql_fetch_array($ergebnis)) {
				...
				$quote = '<input type="button" value="'.$_language->module['quote'].'" onclick="AddCode(\'[quote='.$name.']'.$quotemessage.'[/quote]\')" />';
			...
			if(isfeedbackadmin($userID) || $userID == $id) $submit='<input class="input" type="checkbox" name="ALL" value="ALL" onclick="SelectAll(this.form);" /> '.$_language->module['select_all'].'
											  <input type="submit" value="'.$_language->module['delete_selected'].'" /><br /><br />';
											  else $submit='';
		...
		if(issuperadmin($userID) OR ispageadmin($userID) OR isuseradmin($userID) OR $userID==$ds['userID'] OR $ds['publishprivatedata']==1){
			...
			$postcode=clearfromtags($ds['postcode']);
			if($postcode=='') $postcode=$_language->module['n_a'];
			
			$street=clearfromtags($ds['street']);
			if($street=='') $street=$_language->module['n_a'];
			
			$streetnr=clearfromtags($ds['streetnr']);
			if($streetnr=='') $streetnr=$_language->module['n_a'];
			
			$tel=clearfromtags($ds['tel']);
			if($tel=='') $tel=$_language->module['n_a'];
			
			$fax=clearfromtags($ds['fax']);
			if($fax=='') $fax=$_language->module['n_a'];
			
			$mobile=clearfromtags($ds['mobile']);
			if($mobile=='') $mobile=$_language->module['n_a'];
		}
		else{
			$birthday=$_language->module['hidden'];
			$flag='';
			$profilecountry=$_language->module['hidden'];
			$town='';
			$postcode=$_language->module['hidden'];
			$street=$_language->module['hidden'];
			$streetnr=$_language->module['hidden'];
			$tel=$_language->module['hidden'];
			$fax=$_language->module['hidden'];
			$mobile=$_language->module['hidden'];
		}
