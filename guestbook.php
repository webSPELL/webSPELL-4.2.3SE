...
elseif($action == 'add') {
	...
	while($ds = mysql_fetch_array($ergebnis)) {
		...
		$actions='';
		$ip='logged';
		$quote='<input type="button" value="'.$_language->module['quote'].'" onclick="window.location=\'index.php?site=guestbook&amp;action=add&amp;messageID='.$ds['gbID'].'\'" />';
		if(isfeedbackadmin($userID)) {
		  $actions=' <input type="button" value="'.$_language->module['admin_comment'].'" onclick="window.location=\'index.php?site=guestbook&amp;action=comment&amp;guestbookID='.$ds['gbID'].'\'" /> <input class="input" type="checkbox" name="gbID[]" value="'.$ds['gbID'].'" />';
			$ip=$ds['ip'];
		}
