...
if($action=="faqcat" and is_numeric($_GET['faqcatID'])) {
	...
	$faqcat=safe_query("SELECT question,faqID,sort,viewed FROM ".PREFIX."faq WHERE faqcatID='".$faqcatID."' ORDER BY sort");
	if(mysql_num_rows($faqcat)) {
		...
		while($ds=mysql_fetch_array($faqcat)){
			...
			$viewed=$_language->module['viewed'].': '.$ds['viewed'];
...
elseif($action=="faq") {
  ...
  safe_query("UPDATE ".PREFIX."faq SET viewed = viewed + 1 WHERE faqID='$faqID'");
